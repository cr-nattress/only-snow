...
export default function PromptInput({ scenarios, activeId, onChange }: PromptInputProps) {
  ...
  return (
    <div className="px-4 md:px-6 lg:px-8 pt-3 pb-3">
      <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 lg:px-4 py-2.5 lg:py-3 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500">
        <span className="text-sm lg:text-base text-gray-400 shrink-0">&#x1F50D;</span>
        <input
          ref={inputRef}
          type="text"
          value={query}
          ...
          className="flex-1 bg-transparent text-sm lg:text-base font-medium text-gray-800 placeholder-gray-500 outline-none"
        />
        {query && (
          <button
            onClick={handleSubmit}
            className="shrink-0 bg-blue-600 text-white text-xs lg:text-sm font-bold px-3 lg:px-4 py-1.5 rounded-lg hover:bg-blue-700 active:bg-blue-800 transition-colors"
          >
            Go
          </button>
        )}
