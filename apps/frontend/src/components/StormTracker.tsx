...
const severityStyles: Partial<Record<StormSeverity, { bg: string; border: string; text: string; icon: string; label: string }>> = {
  quiet: {
    bg: "bg-gray-50",
...
const defaultStyle = { bg: "bg-gray-50", border: "border-gray-200", text: "text-gray-500", icon: "", label: "STORMS" };

export default function StormTracker({ storm }: StormTrackerProps) {
  const style = severityStyles[storm.severity] ?? defaultStyle;
